...
rootGrid.RowHeight = {84,'1x'};
rootGrid.Padding = [10 10 10 10];

% ------------------------- common audio bar -----------------------------
commonBar = uigridlayout(rootGrid,[2 7]);
commonBar.Layout.Row = 1;
commonBar.RowHeight = {32,28};
commonBar.ColumnWidth = {105,105,110,95,105,100,'1x'};
...
helpLabel = uilabel(commonBar,'Text','课堂顺序：采样与混叠 → 傅里叶变换 → 加噪 → 滤波 → 传统变声');
helpLabel.Layout.Row = 2; helpLabel.Layout.Column = [1 6];
statusLabel = uilabel(commonBar,'Text','就绪','HorizontalAlignment','right','Tag','statusLabel');
statusLabel.Layout.Row = 2; statusLabel.Layout.Column = 7;

% ----------------------------- tabs ------------------------------------
tg = uitabgroup(rootGrid); tg.Layout.Row = 2;
tabSampling = uitab(tg,'Title','1  信号采样与混叠');
tabFFT = uitab(tg,'Title','2  傅里叶变换');
tabFilter = uitab(tg,'Title','3  加噪与滤波');
tabVoice = uitab(tg,'Title','4  频谱/传统变声');

% ------------------------ sampling tab ---------------------------------
sg = uigridlayout(tabSampling,[2 3]);
sg.ColumnWidth = {280,'1x','1x'}; sg.RowHeight = {'1x','1x'};
sp = uipanel(sg,'Title','课堂实验'); sp.Layout.Row = [1 2]; sp.Layout.Column = 1;
spg = uigridlayout(sp,[20 2]);
spg.RowHeight = repmat({27},1,20); spg.ColumnWidth = {130,'1x'};
sectionA = uilabel(spg,'Text','实验 A：正弦采样与混叠','FontWeight','bold'); sectionA.Layout.Row=1; sectionA.Layout.Column=[1 2];
lab=uilabel(spg,'Text','教学正弦频率/Hz'); lab.Layout.Row=2; lab.Layout.Column=1;
sf0=uieditfield(spg,'numeric','Value',900,'Limits',[1 Inf],'Tag','samplingF0'); sf0.Layout.Row=2; sf0.Layout.Column=2;
lab=uilabel(spg,'Text','采样率/Hz'); lab.Layout.Row=3; lab.Layout.Column=1;
sfs=uieditfield(spg,'numeric','Value',1000,'Limits',[1 Inf],'Tag','samplingFs'); sfs.Layout.Row=3; sfs.Layout.Column=2;
lab=uilabel(spg,'Text','观察时长/s'); lab.Layout.Row=4; lab.Layout.Column=1;
sdur=uieditfield(spg,'numeric','Value',0.02,'Limits',[0.005 2],'Tag','samplingDuration'); sdur.Layout.Row=4; sdur.Layout.Column=2;
countLabel=uilabel(spg,'Text','采样点数 N：自动计算','Tag','sampleCountLabel'); countLabel.Layout.Row=5; countLabel.Layout.Column=[1 2];
runSine=uibutton(spg,'Text','运行正弦采样实验','ButtonPushedFcn',@onSamplingDemo); runSine.Layout.Row=6; runSine.Layout.Column=[1 2];
sectionB=uilabel(spg,'Text','实验 B：语音降采样（每隔 M 点保留 1 点）','FontWeight','bold','WordWrap','on'); sectionB.Layout.Row=7; sectionB.Layout.Column=[1 2];
lab=uilabel(spg,'Text','降采样因子 M'); lab.Layout.Row=8; lab.Layout.Column=1;
sdec=uieditfield(spg,'numeric','Value',4,'RoundFractionalValues','on','Limits',[1 20],'Tag','speechDecimationFactor'); sdec.Layout.Row=8; sdec.Layout.Column=2;
runDec=uibutton(spg,'Text','执行语音降采样','ButtonPushedFcn',@onSpeechDecimate); runDec.Layout.Row=9; runDec.Layout.Column=[1 2];
playDec=uibutton(spg,'Text','播放降采样后语音','ButtonPushedFcn',@onPlayProcessed); playDec.Layout.Row=10; playDec.Layout.Column=[1 2];
...
% -------------------------- Fourier tab --------------------------------
fg=uigridlayout(tabFFT,[2 3]); fg.ColumnWidth={220,'1x','1x'}; fg.RowHeight={'1x','1x'};
fp=uipanel(fg,'Title','分析设置'); fp.Layout.Row=[1 2]; fp.Layout.Column=1;
fpg=uigridlayout(fp,[10 2]); fpg.RowHeight=repmat({30},1,10); fpg.ColumnWidth={90,'1x'};
uilabel(fpg,'Text','分析时长/s'); fsec=uieditfield(fpg,'numeric','Value',2,'Limits',[0.05 Inf],'Tag','fftDuration');
runFFT=uibutton(fpg,'Text','分析当前语音','ButtonPushedFcn',@onFourier); runFFT.Layout.Column=[1 2];
fTip=uitextarea(fpg,'Editable','off','Value',{'上图看时间变化；右上看整体频率成分；下图看频率成分随时间变化。';'语谱图由程序自行分帧 FFT，不依赖 spectrogram 工具箱函数。'}); fTip.Layout.Row=[4 10]; fTip.Layout.Column=[1 2];
...
% ----------------------- noise + filter tab -----------------------------
flg=uigridlayout(tabFilter,[2 4]); flg.ColumnWidth={280,'1x','1x','1x'}; flg.RowHeight={'1x','1x'};
flp=uipanel(flg,'Title','步骤 1 加噪；步骤 2 滤波'); flp.Layout.Row=[1 2]; flp.Layout.Column=1;
flpg=uigridlayout(flp,[19 2]); flpg.RowHeight=repmat({27},1,19); flpg.ColumnWidth={115,'1x'};
uilabel(flpg,'Text','加噪模式'); noiseDD=uidropdown(flpg,'Items',{'单频干扰','混合噪声','白噪声','无'},'Value','单频干扰','Tag','noiseMode');
uilabel(flpg,'Text','噪声强度'); noiseAmt=uieditfield(flpg,'numeric','Value',0.50,'Limits',[0 2],'Tag','noiseAmount');
uilabel(flpg,'Text','干扰频率/Hz'); toneField=uieditfield(flpg,'numeric','Value',1000,'Limits',[1 Inf],'Tag','interferenceFrequency');
addNoiseBtn=uibutton(flpg,'Text','步骤 1：生成含噪语音','ButtonPushedFcn',@onGenerateNoise); addNoiseBtn.Layout.Column=[1 2];
playNoisy=uibutton(flpg,'Text','播放含噪语音','ButtonPushedFcn',@onPlayNoisy); playNoisy.Layout.Column=[1 2];
uilabel(flpg,'Text','滤波器'); filterDD=uidropdown(flpg,'Items',{'带阻','低通','高通','带通','音质优化'},'Value','带阻','Tag','filterType');
uilabel(flpg,'Text','频率1/Hz'); ff1=uieditfield(flpg,'numeric','Value',900,'Limits',[1 Inf],'Tag','filterF1');
uilabel(flpg,'Text','频率2/Hz'); ff2=uieditfield(flpg,'numeric','Value',1100,'Limits',[1 Inf],'Tag','filterF2');
uilabel(flpg,'Text','FIR 阶数'); forder=uieditfield(flpg,'numeric','Value',200,'RoundFractionalValues','on','Limits',[20 800],'Tag','filterOrder');
applyFilterBtn=uibutton(flpg,'Text','步骤 2：应用滤波器','ButtonPushedFcn',@onApplyFilter); applyFilterBtn.Layout.Column=[1 2];
playFiltered=uibutton(flpg,'Text','播放滤波结果','ButtonPushedFcn',@onPlayFiltered); playFiltered.Layout.Column=[1 2];
filterMetricLabel=uilabel(flpg,'Text','目标频点衰减：尚未滤波','Tag','filterMetricLabel','WordWrap','on'); filterMetricLabel.Layout.Row=[12 13]; filterMetricLabel.Layout.Column=[1 2]; filterMetricLabel.UserData=[];
filterTip=uitextarea(flpg,'Editable','off','Value',{'单频干扰的教学流程：先加噪，再滤波。';'选择“带阻”时，生成单频干扰后会自动把阻带调整到干扰频率附近。';'混合噪声还含白噪声，窄带阻只能去掉其中的单频分量。'}); filterTip.Layout.Row=[14 19]; filterTip.Layout.Column=[1 2];
axFilterTime=uiaxes(flg,'Tag','noiseTimeAxes'); axFilterTime.Layout.Row=1; axFilterTime.Layout.Column=[2 3]; axFilterTime.Title.String='步骤 1：原音与含噪语音时域对比'; axFilterTime.XLabel.String='时间 / s'; grid(axFilterTime,'on');
axResponse=uiaxes(flg,'Tag','filterResponseAxes'); axResponse.Layout.Row=1; axResponse.Layout.Column=4; axResponse.Title.String='步骤 2：滤波器幅频响应'; axResponse.XLabel.String='频率 / Hz'; axResponse.YLabel.String='dB'; grid(axResponse,'on');
axBeforeSpec=uiaxes(flg,'Tag','noiseFilterSpectrumAxes'); axBeforeSpec.Layout.Row=2; axBeforeSpec.Layout.Column=[2 3]; axBeforeSpec.Title.String='原音、含噪与滤波后频谱'; axBeforeSpec.XLabel.String='频率 / Hz'; grid(axBeforeSpec,'on');
axMetric=uiaxes(flg,'Tag','filterMetricAxes'); axMetric.Layout.Row=2; axMetric.Layout.Column=4; axMetric.Title.String='目标干扰频点'; axMetric.YLabel.String='幅值'; grid(axMetric,'on');

% --------------------------- voice tab ----------------------------------
vg=uigridlayout(tabVoice,[2 3]); vg.ColumnWidth={255,'1x','1x'}; vg.RowHeight={'1x','1x'};
vp=uipanel(vg,'Title','变声设置'); vp.Layout.Row=[1 2]; vp.Layout.Column=1;
vpg=uigridlayout(vp,[13 2]); vpg.RowHeight=repmat({30},1,13); vpg.ColumnWidth={90,'1x'};
uilabel(vpg,'Text','效果'); voiceDD=uidropdown(vpg,'Items',{'升调','降调','机器人','明亮'},'Value','升调','Tag','voiceEffect');
runVoice=uibutton(vpg,'Text','应用传统 DSP 变声','ButtonPushedFcn',@onVoice); runVoice.Layout.Column=[1 2];
playVoice=uibutton(vpg,'Text','播放变声结果','ButtonPushedFcn',@onPlayVoice); playVoice.Layout.Column=[1 2];
voiceDesc=uitextarea(vpg,'Editable','off','Value',{'传统 DSP 变声用于解释音调、频谱与调制，不等同于 AI 声线克隆。'}); voiceDesc.Layout.Row=[5 11]; voiceDesc.Layout.Column=[1 2];
aiNote=uilabel(vpg,'Text','AI 声线克隆：高级扩展，暂不作为主体依赖','WordWrap','on'); aiNote.Layout.Row=[12 13]; aiNote.Layout.Column=[1 2];
axVoiceTime=uiaxes(vg); axVoiceTime.Layout.Row=1; axVoiceTime.Layout.Column=[2 3]; axVoiceTime.Title.String='变声前后时域'; axVoiceTime.XLabel.String='时间 / s'; grid(axVoiceTime,'on');
axVoiceBefore=uiaxes(vg); axVoiceBefore.Layout.Row=2; axVoiceBefore.Layout.Column=2; axVoiceBefore.Title.String='变声前频谱'; axVoiceBefore.XLabel.String='频率 / Hz'; grid(axVoiceBefore,'on');
axVoiceAfter=uiaxes(vg); axVoiceAfter.Layout.Row=2; axVoiceAfter.Layout.Column=3; axVoiceAfter.Title.String='变声后频谱'; axVoiceAfter.XLabel.String='频率 / Hz'; grid(axVoiceAfter,'on');
...
% ============================= callbacks ================================
    function onImport(~,~)
        [file,path]=uigetfile({'*.wav;*.mp3;*.m4a','音频文件 (*.wav, *.mp3, *.m4a)';'*.*','所有文件'});
        if isequal(file,0), return; end
        try
            [x,fs]=audioread(fullfile(path,file));
            audio=make_audio_struct(x,fs,file);
            clearDerived(); updateAudioInfo(); onFourier([],[]);
            setStatus(sprintf('已导入并切换当前音频：%s',file));
        catch ME, showError(ME); end
    end

    function onRecord(~,~)
        try
            setStatus('正在录音 3 秒...'); drawnow;
            rec=audiorecorder(16000,16,1); recordblocking(rec,3);
            x=getaudiodata(rec,'double');
            audio=make_audio_struct(x,16000,'现场录音');
            clearDerived(); updateAudioInfo(); onFourier([],[]);
            setStatus('录音完成，当前原音已切换为现场录音');
        catch ME, showError(ME); end
    end

    function onPlayOriginal(~,~)
        % Read the shared audio variable at CLICK time.  Do not capture the
        % startup demo waveform in an anonymous callback.
        playSignal(audio.x,audio.fs);
        setStatus(sprintf('正在播放当前原音：%s',audio.name));
    end
    ...
    function onRunSelfTest(~,~)
        ...
        try
            ...
            uialert(fig,{'核心数值算法自检全部通过。';'采样混叠、FFT、FIR 滤波和传统变声测试均通过。'},'自检通过','Icon','info');
        catch ME
            setStatus('自检失败');
    ...
    function onSpeechDecimate(~,~)
        try
            ...
            audio.processed=normalize_audio(y); audio.processedFs=fsNew; audio.processedName='语音降采样结果';
            [f0,m0]=one_sided_spectrum(audio.x,audio.fs); [f1,m1]=one_sided_spectrum(y,fsNew);
            cla(axSampleSpec); hold(axSampleSpec,'on'); plot(axSampleSpec,f0,m0,'DisplayName','原始'); plot(axSampleSpec,f1,m1,'DisplayName','每隔 M 点保留 1 点后'); hold(axSampleSpec,'off'); legend(axSampleSpec,'Location','best'); xlim(axSampleSpec,[0 min(audio.fs,fsNew)/2]);
            sInfo.Value={'实验 B：语音降采样'; sprintf('原采样率：%.1f Hz',audio.fs); sprintf('降采样因子 M：%d',factor); sprintf('含义：每隔 %d 个原始样本只保留 1 个。',factor); sprintf('降采样后采样率：%.1f Hz',fsNew); sprintf('新奈奎斯特频率：%.1f Hz',fsNew/2); '这里故意不做抗混叠滤波，用于课堂观察错误降采样造成的频谱折叠和听感变化。'; '工程中的正规降采样应先低通抗混叠，再降低采样率。'};
            setStatus(sprintf('语音降采样完成：%.0f Hz → %.0f Hz（M=%d）',audio.fs,fsNew,factor));
        catch ME, showError(ME); end
    end

    function onPlayProcessed(~,~)
        if isempty(audio.processed), uialert(fig,'请先执行语音降采样。','尚无结果'); return; end
        playSignal(audio.processed,audio.processedFs); setStatus('正在播放降采样后语音');
    end
    ...
    function onFourier(~,~)
        try
            ...
            fTip.Value={sprintf('当前分析：%s，%.2f s',audio.name,N/audio.fs); sprintf('整体单边谱最强非直流频率约 %.1f Hz',dominant); '时域波形：看振幅随时间变化。'; '单边幅度谱：看整个分析时段的总体频率组成。'; '短时频谱：看不同频率何时出现。'};
    ...
    function showNoisePlots(xOriginal,xNoisy,tone)
    ...
    function updateAudioInfo()
        infoLabel.Text=sprintf('当前音频：%s | %.0f Hz | %.2f s',audio.name,audio.fs,numel(audio.x)/audio.fs);
    end
    ...
    function playSignal(x,fs)
        onStopSound([],[]); player=audioplayer(normalize_audio(x),fs); play(player);
    end
