...
rootGrid = uigridlayout(fig, [2 1]); rootGrid.RowHeight = {78, '1x'}; rootGrid.Padding = [10 10 10 10];
commonBar = uigridlayout(rootGrid, [2 8]); commonBar.Layout.Row = 1; commonBar.RowHeight = {32, 28}; commonBar.ColumnWidth = {105,105,110,95,105,120,'1x',120};
...
uibutton(commonBar,'Text','运行自检提示','ButtonPushedFcn',@onTestHint);
infoLabel=uilabel(commonBar,'Text','','FontWeight','bold'); infoLabel.Layout.Row=1; infoLabel.Layout.Column=7;
statusLabel=uilabel(commonBar,'Text','就绪','HorizontalAlignment','right'); statusLabel.Layout.Row=1; statusLabel.Layout.Column=8;
helpLabel=uilabel(commonBar,'Text','课堂顺序建议：采样与混叠 → FFT → 加噪与滤波 → 传统变声'); helpLabel.Layout.Row=2; helpLabel.Layout.Column=[1 8];
...
sg=uigridlayout(tabSampling,[2 3]); sg.ColumnWidth={245,'1x','1x'}; sg.RowHeight={'1x','1x'};
sp=uipanel(sg,'Title','参数与操作'); sp.Layout.Row=[1 2]; sp.Layout.Column=1; spg=uigridlayout(sp,[15 2]); spg.RowHeight=repmat({28},1,15); spg.ColumnWidth={105,'1x'};
uilabel(spg,'Text','教学正弦频率/Hz'); sf0=uieditfield(spg,'numeric','Value',900,'Limits',[1 Inf]);
uilabel(spg,'Text','采样率/Hz'); sfs=uieditfield(spg,'numeric','Value',1000,'Limits',[1 Inf]);
uilabel(spg,'Text','观察时长/s'); sdur=uieditfield(spg,'numeric','Value',0.10,'Limits',[0.005 2]);
runSine=uibutton(spg,'Text','运行正弦采样实验','ButtonPushedFcn',@onSamplingDemo); runSine.Layout.Column=[1 2];
uilabel(spg,'Text','语音直接抽取因子'); sdec=uieditfield(spg,'numeric','Value',4,'RoundFractionalValues','on','Limits',[1 20]);
runDec=uibutton(spg,'Text','对当前语音直接抽取','ButtonPushedFcn',@onSpeechDecimate); runDec.Layout.Column=[1 2];
playDec=uibutton(spg,'Text','播放抽取后语音','ButtonPushedFcn',@onPlayProcessed); playDec.Layout.Column=[1 2];
sInfo=uitextarea(spg,'Editable','off','Value',{'改变采样率，观察奈奎斯特条件和混叠频率。';'语音“直接抽取”故意不做抗混叠滤波，只用于教学。'}); sInfo.Layout.Row=[8 15]; sInfo.Layout.Column=[1 2];
axSample=uiaxes(sg); axSample.Layout.Row=1; axSample.Layout.Column=[2 3]; axSample.Title.String='采样点与表观混叠波形'; axSample.XLabel.String='时间 / s'; axSample.YLabel.String='幅值'; grid(axSample,'on');
axSampleSpec=uiaxes(sg); axSampleSpec.Layout.Row=2; axSampleSpec.Layout.Column=[2 3]; axSampleSpec.Title.String='采样序列的单边幅度谱'; axSampleSpec.XLabel.String='频率 / Hz'; axSampleSpec.YLabel.String='幅值'; grid(axSampleSpec,'on');
...
fpg=uigridlayout(fp,[10 2]); fpg.RowHeight=repmat({30},1,10); fpg.ColumnWidth={90,'1x'}; uilabel(fpg,'Text','分析时长/s'); fsec=uieditfield(fpg,'numeric','Value',2,'Limits',[0.05 Inf]);
...
axTime=uiaxes(fg); axTime.Layout.Row=1; axTime.Layout.Column=2; axTime.Title.String='时域波形'; axTime.XLabel.String='时间 / s'; grid(axTime,'on');
axFFT=uiaxes(fg); axFFT.Layout.Row=1; axFFT.Layout.Column=3; axFFT.Title.String='单边幅度谱'; axFFT.XLabel.String='频率 / Hz'; grid(axFFT,'on');
axSpecgram=uiaxes(fg); axSpecgram.Layout.Row=2; axSpecgram.Layout.Column=[2 3]; axSpecgram.Title.String='短时频谱（语谱图）'; axSpecgram.XLabel.String='时间 / s'; axSpecgram.YLabel.String='频率 / Hz';
...
uilabel(flpg,'Text','加噪模式'); noiseDD=uidropdown(flpg,'Items',{'混合噪声','单频干扰','白噪声','无'},'Value','单频干扰');
uilabel(flpg,'Text','噪声强度'); noiseAmt=uieditfield(flpg,'numeric','Value',0.35,'Limits',[0 2]); uilabel(flpg,'Text','干扰频率/Hz'); toneField=uieditfield(flpg,'numeric','Value',1000,'Limits',[1 Inf]);
uilabel(flpg,'Text','滤波器'); filterDD=uidropdown(flpg,'Items',{'带阻','低通','高通','带通','音质优化'},'Value','带阻'); uilabel(flpg,'Text','频率1/Hz'); ff1=uieditfield(flpg,'numeric','Value',900,'Limits',[1 Inf]); uilabel(flpg,'Text','频率2/Hz'); ff2=uieditfield(flpg,'numeric','Value',1100,'Limits',[1 Inf]); uilabel(flpg,'Text','FIR 阶数'); forder=uieditfield(flpg,'numeric','Value',200,'RoundFractionalValues','on','Limits',[20 800]);
...
axFilterTime=uiaxes(flg); axFilterTime.Layout.Row=1; axFilterTime.Layout.Column=[2 3]; axFilterTime.Title.String='处理前后时域对比'; axFilterTime.XLabel.String='时间 / s'; grid(axFilterTime,'on'); axResponse=uiaxes(flg); axResponse.Layout.Row=1; axResponse.Layout.Column=4; axResponse.Title.String='滤波器幅频响应'; axResponse.XLabel.String='频率 / Hz'; axResponse.YLabel.String='dB'; grid(axResponse,'on'); axBeforeSpec=uiaxes(flg); axBeforeSpec.Layout.Row=2; axBeforeSpec.Layout.Column=[2 3]; axBeforeSpec.Title.String='含噪与处理后频谱'; axBeforeSpec.XLabel.String='频率 / Hz'; grid(axBeforeSpec,'on'); axMetric=uiaxes(flg); axMetric.Layout.Row=2; axMetric.Layout.Column=4; axMetric.Title.String='干扰频点处理前后'; axMetric.YLabel.String='幅值'; grid(axMetric,'on');
...
uilabel(vpg,'Text','效果'); voiceDD=uidropdown(vpg,'Items',{'升调','降调','机器人','明亮'},'Value','升调'); runVoice=uibutton(vpg,'Text','应用传统 DSP 变声','ButtonPushedFcn',@onVoice); runVoice.Layout.Column=[1 2]; playVoice=uibutton(vpg,'Text','播放变声结果','ButtonPushedFcn',@onPlayVoice); playVoice.Layout.Column=[1 2]; voiceDesc=uitextarea(vpg,'Editable','off','Value',{'第一版不把传统 DSP 变声称为“AI 声线转换”。'}); voiceDesc.Layout.Row=[5 11]; voiceDesc.Layout.Column=[1 2]; aiNote=uilabel(vpg,'Text','AI 声线克隆：高级扩展，暂不作为主体依赖','WordWrap','on'); aiNote.Layout.Row=[12 13]; aiNote.Layout.Column=[1 2];
...
updateAudioInfo(); onSamplingDemo([],[]); onFourier([],[]);
    ...
    function onTestHint(~,~), uialert(fig,sprintf(['算法自检入口：\n\ncd(''%s'')\naddpath(''tests'')\nrun_all_tests\n\n命令行应显示 All DSP teaching platform tests passed.'],projectRoot),'自检方法'); end
    function onSamplingDemo(~,~)
        try, d=sine_sampling_demo(sf0.Value,sfs.Value,sdur.Value); cla(axSample); hold(axSample,'on'); plot(axSample,d.tRef,d.xRef,'LineWidth',1.0,'DisplayName','原始连续参考'); plot(axSample,d.tRef,d.xApparent,'--','LineWidth',1.2,'DisplayName',sprintf('表观 %.1f Hz',d.aliasHz)); stem(axSample,d.tSample,d.xSample,'filled','DisplayName','采样点'); hold(axSample,'off'); legend(axSample,'Location','best'); [f,m]=one_sided_spectrum(d.xSample,d.fs); plot(axSampleSpec,f,m,'LineWidth',1.2); xlim(axSampleSpec,[0 d.fs/2]); if d.isUndersampled, verdict='欠采样：fs < 2f0，发生混叠'; else, verdict='满足奈奎斯特条件：fs >= 2f0'; end; sInfo.Value={verdict;sprintf('f0 = %.1f Hz, fs = %.1f Hz, Nyquist = %.1f Hz',d.f0,d.fs,d.nyquist);sprintf('观测到的基带频率约为 %.1f Hz',d.aliasHz);'语音直接抽取同样不使用抗混叠滤波，仅用于观察现象。'}; catch ME, showError(ME); end
    end
    function onSpeechDecimate(~,~)
        try, factor=max(1,round(sdec.Value)); [y,fsNew]=naive_decimate_audio(audio.x,audio.fs,factor); audio.processed=normalize_audio(y); audio.processedFs=fsNew; audio.processedName='直接抽取结果'; [f0,m0]=one_sided_spectrum(audio.x,audio.fs); [f1,m1]=one_sided_spectrum(y,fsNew); cla(axSampleSpec); hold(axSampleSpec,'on'); plot(axSampleSpec,f0,m0,'DisplayName','原始'); plot(axSampleSpec,f1,m1,'DisplayName','直接抽取后'); hold(axSampleSpec,'off'); legend(axSampleSpec,'Location','best'); xlim(axSampleSpec,[0 min(audio.fs,fsNew)/2]); setStatus(sprintf('直接抽取完成，新采样率 %.1f Hz',fsNew)); catch ME, showError(ME); end
    end
    ...
    function onFourier(~,~)
        try, N=min(numel(audio.x),max(16,round(fsec.Value*audio.fs))); x=audio.x(1:N); t=(0:N-1)'/audio.fs; plot(axTime,t,x); xlim(axTime,[t(1) max(t(end),1/audio.fs)]); [f,m]=one_sided_spectrum(x,audio.fs); plot(axFFT,f,m); xlim(axFFT,[0 audio.fs/2]); [S,ff,tt]=frame_spectrogram(x,audio.fs,30,0.75); imagesc(axSpecgram,tt,ff,20*log10(S+eps)); axis(axSpecgram,'xy'); ylim(axSpecgram,[0 min(5000,audio.fs/2)]); colorbar(axSpecgram); setStatus('FFT 与短时频谱已更新'); catch ME, showError(ME); end
    end
    function onFilter(~,~)
